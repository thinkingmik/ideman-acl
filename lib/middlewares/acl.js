...
var isAllowed = function(resource, permission) {
  return function(req, res, callback) {
    ...
    .then(function(policies) {
      ...
      return callback(new AclNotFoundError());
    })
    .catch(function(err) {
      return callback(new AclNotFoundError(err.message));
    });
  }
}
...
var getUserId = function(req) {
  ...
  return new Promise(function(resolve, reject) {
    if (credentials == null) {
      ...
      return AutoLoader.getIdemanModel(idemanDbConfig.entities.token.model).forge()
      .where({
        token: bearerToken
      })
      .fetch({
